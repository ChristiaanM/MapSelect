...
#ifndef MAPSELECT_ALIAS_H
#define MAPSELECT_ALIAS_H
...
#define ID_ERROR std::numeric_limits<size_t>::max()

    /*! @brief Implements a mapping from ID to Key. Used for MapDataAlias

    Alias is a class that implements a mapping from a set of integer values to a key which is integer values that go from 0 to n,
    ...
    */
    ...
    {
    public:
        AliasIdentity(size_t max_id) : identity_max(max_id)
        {
        }
        ...
        size_t size() const { return identity_max; }
        size_t getLastId() const { return identity_max; }
